...
REM standalone programs (bundling Python + pymobiledevice3) under helper-bin\.
REM Uses --onedir (folder per program) for FAST startup: no per-launch
REM unpack like --onefile, so tunneld/helper spawn in ~1-2s instead of ~8s.
REM Output: helper-bin\ios-location-helper\ios-location-helper.exe
REM         helper-bin\tunneld\tunneld.exe
REM Then "npm run dist:win" packs helper-bin\ into resources\helper\.
...
echo.
echo Done: helper-bin\ios-location-helper\ios-location-helper.exe + helper-bin\tunneld\tunneld.exe
